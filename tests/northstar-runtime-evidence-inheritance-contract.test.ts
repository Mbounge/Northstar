...
const workspace = fs.readFileSync(
  path.join(process.cwd(), "lib/canvas-ai/northstar-creative-source-workspace.ts"),
  "utf8",
);
...
test("protected evidence survival is runtime-owned across the complete creative path", () => {
  ...
  assert.equal(workspace.includes("the runtime inherits every other protected evidence node"), true);
