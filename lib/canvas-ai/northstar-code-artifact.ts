...
function alignDeterministicOperationsToDocument(
  ...
  const protectedSceneTargets = new Set(["artboard", "header"]);
  return operations.filter((operation) => {
    if (operation.op === "set-css-layer") return true;
    if ((operation.op === "set-html" || operation.op === "recompose-region") && protectedSceneTargets.has(operation.targetId)) return false;
    ...
    if (operation.op === "recompose-region") {
      const introducedIds = [...operation.html.matchAll(/data-ns-node-id=["']([^"']+)["']/g)].map((match) => match[1]);
      for (const nodeId of introducedIds) available.add(nodeId);
      if (operation.placements.some((placement) => !available.has(placement.targetId))) return false;
      if (operation.placements.some((placement) => placement.parentId !== operation.targetId && !available.has(placement.parentId))) return false;
    }
    return true;
  });
}
...
export function createNorthstarWorkingMutationPackage(input: {
  ...
  operations.push({
    ...
    css: input.phase === "evidence"
      ? `[data-ns-node-id="evidence"]{gap:30px}[data-ns-node-id="header"]{padding-bottom:10px}.working-flow{background:transparent;border-radius:0;box-shadow:none}.working-flow__sequence{display:flex!important;flex-direction:row!important;flex-wrap:nowrap!important;width:100%!important;max-width:100%!important;gap:18px!important;overflow:visible!important}.working-flow__sequence>figure{flex:1 1 0!important;width:auto!important;min-width:92px!important;max-width:none!important}`
