// app/api/apps/[companyId]/flows/route.ts
import { NextResponse } from "next/server";
import { createClient } from "@/lib/supabase/server";
import { getAppFlowsData } from "@/lib/review-data";
import {
  ...
  try {
    ...
    const { value: data, status: cacheStatus } =
      await getOrSetServerResponseCache(cacheKey, () =>
        getAppFlowsData(appName, profile.customer_id, platform, mode)
      );
    ...
    return NextResponse.json(
      { ok: false, error: "Failed to load flows data" },
      { status: 500 }
    );
  }
}