...
export function CanvasV2ResearchPanel({ endpoint, busy, onInsertFlow, onInsertScreen }: {
  ...
  return <div className="flex min-h-0 flex-1 flex-col">
    <div className="border-b border-[#ececf3] p-5 dark:border-white/[.08]">
      <div className="flex items-center gap-2 rounded-xl bg-[#f5f4fa] px-3 py-2.5 dark:bg-white/[.055]">
        <Search className="h-4 w-4 text-[#9292a1]" />
        <input aria-label="Search research evidence" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && search()} placeholder="Search apps, flows, screens…" className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-[#9292a1] dark:text-[#f1eff6]" />
      </div>
    </div>
    <div className="min-h-0 flex-1 overflow-y-auto p-5">
      ...
      {research.error && <div className="rounded-xl bg-red-50 p-3 text-xs leading-5 text-red-700 dark:bg-red-500/[.1] dark:text-red-300">{research.error}</div>}
      {!research.loading && !research.error && showingScreens && <>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">{research.result?.screens.map((screen, index) => <button key={screen.id} onClick={() => onInsertScreen(research.result!, index)} disabled={busy || !screen.imageUrl} className="group overflow-hidden rounded-xl border border-[#e6e6ee] bg-[#fafafe] text-left disabled:opacity-40 dark:border-white/[.09] dark:bg-white/[.035]"><div className="grid h-36 place-items-center overflow-hidden bg-white p-2 dark:bg-[#24222b]">{screen.imageUrl ? <img src={screen.imageUrl} alt={screen.name} className="h-full w-full object-contain transition group-hover:scale-[1.03]" /> : <Images className="h-5 w-5 text-[#aaaab8]" />}</div><div className="border-t border-[#ececf3] p-2 dark:border-white/[.08]"><div className="truncate text-[11px] font-bold">{index + 1}. {screen.name}</div><div className="mt-0.5 text-[10px] text-[#9292a1]">Click to insert</div></div></button>)}</div>
      </>}
    ...
    <div className="border-t border-[#ececf3] px-5 py-3 text-[10px] leading-4 text-[#9696a4] dark:border-white/[.08] dark:text-[#777482]">Inserted media keeps its exact account evidence identity, URL, app, flow, and order.</div>
  </div>;
}
