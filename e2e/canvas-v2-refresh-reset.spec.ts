...
test("refresh discards committed artboard, chat, and undo history", async ({ page }) => {
...
test("refresh during active work cancels it and opens a completely clean session", async ({ page }) => {
  ...
  await expect(page.getByText("Stopped. The latest committed artboard remains visible.")).toHaveCount(0);
