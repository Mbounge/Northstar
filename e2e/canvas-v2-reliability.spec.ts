...
test("routing retries one transient provider failure without touching the artboard", async ({ page }) => {
  ...
  await expect(page.getByText("Connection interrupted — retrying request 2 of 3…")).toBeVisible();
...
test("exhausted retries fail without publishing a candidate", async ({ page }) => {
  ...
  await expect(page.getByTestId("canvas-v2-committed-revision")).toHaveText(before ?? "");
  await expect(page.locator('[data-testid="canvas-v2-preview"]').contentFrame().locator("[data-e2e-retry-revision]")).toHaveCount(0);
